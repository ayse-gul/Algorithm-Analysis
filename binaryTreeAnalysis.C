...
int toplam_dinamik( struct dugum *kok, struct dugum *t, struct dugum *d){
    int static i=0;
    if(d==NULL) return i;
    i=i+uzunluk(kok,t,d);
    t=kok;
    toplam_dinamik(kok,t,d->sol);
    toplam_dinamik(kok,t,d->sag);
}
...
int rastgele_sayi(int kac_arasinda){
    int sayi;
...
int main(int argc, char** argv) {
    ...
    */
    ...
    for(i=0;i<kac_dugumlu;i++){
        ...
        sayi=rastgele_sayi(1000);
        ekle(agac,sayi);
    }

    preorder(agac);

    a=toplam_dinamik(agac->kok,agac->kok,agac->kok);

    ortalama=(float)a/kac_dugumlu;
    printf("\n\nderinlik:%d\n\n",a);

    printf("\n\nortalama derinlik = toplam derinlik/dugum sayisi = %d/%d\n\n\n\n",a,kac_dugumlu);
    printf("=%f\n",ortalama);
